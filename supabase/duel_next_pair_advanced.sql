-- Advanced duel_next_pair: Elo proximity, info gain, cooldown, no repeats.
...
DECLARE
  ...
  v_cooldown int := 20;
BEGIN
  -- A: user_wine_stats, recent_wines, already_compared, then pick A
  WITH
  uws AS (
    SELECT
      w.id AS wid,
      COALESCE(r.elo_score, 1500.0) AS elo,
  ...
  ac AS (
    SELECT DISTINCT
      LEAST(comp.wine_a_id, comp.wine_b_id) AS lo,
      GREATEST(comp.wine_a_id, comp.wine_b_id) AS hi
  ...
  SELECT
    s.id, s.name, s.producer, s.vintage, s.region, s.label_image_url, s.category,
    s.elo, s.n, s.is_new
  INTO v_a_id, v_a_name, v_a_producer, v_a_vintage, v_a_region, v_a_label, v_a_cat,
       v_a_elo, v_a_n, v_is_new
  FROM sel_a s;
  ...
  -- B: same CTEs, candidate_b, scored_b
  WITH
  uws2 AS (
  ...
  cb AS (
    SELECT
      w.id AS wbid,
      w.name AS wbname,
      w.producer AS wbprod,
      w.vintage AS wbvin,
      w.region AS wbreg,
      w.label_image_url AS wblab,
      u.elo AS elob,
      u.n AS nb,
    ...
    WHERE w.id <> v_a_id
      AND (w.category IS NOT DISTINCT FROM v_a_cat)
      AND ac2.lo IS NULL
      AND ABS(v_a_elo - u.elo) <= 400.0
  ),
  sb AS (
    SELECT wbid, wbname, wbprod, wbvin, wbreg, wblab,
           (0.70 * elo_close + 0.30 * infogain - 1.00 * cooldown) AS sc
    FROM cb
  )
  ...
  -- Fallback B: relax cooldown (same score formula, lower cooldown penalty)
  IF v_b_id IS NULL THEN
    ...
    cb3 AS (
      SELECT w.id AS wbid, w.name AS wbname, w.producer AS wbprod, w.vintage AS wbvin, w.region AS wbreg, w.label_image_url AS wblab,
             ...
             EXP(-ABS(v_a_elo - u.elo) / v_tau) AS elo_close,
             (1.0 / SQRT(1.0 + u.n)) AS infogain,
             CASE WHEN rw3.wine_id IS NOT NULL THEN 0.5 ELSE 0.0 END AS cooldown
      FROM public.wines w
      JOIN uws3 u ON u.wid = w.id
    ...
    SELECT sb3.wbid, sb3.wbname, sb3.wbprod, sb3.wbvin, sb3.wbreg, sb3.wblab
    INTO v_b_id, v_b_name, v_b_producer, v_b_vintage, v_b_region, v_b_label
    FROM sb3 ORDER BY sb3.sc DESC LIMIT 1;
  END IF;

  -- Fallback B: relax repeats (last 30 days only)
  IF v_b_id IS NULL THEN
    WITH
    ac30 AS (
      SELECT DISTINCT LEAST(comp.wine_a_id, comp.wine_b_id) AS lo, GREATEST(comp.wine_a_id, comp.wine_b_id) AS hi
      FROM public.comparisons comp
      WHERE comp.user_id = p_user_id AND comp.created_at >= NOW() - INTERVAL '30 days'
    ),
    uws4 AS (
      SELECT w.id AS wid, COALESCE(r.elo_score, 1500.0) AS elo, COALESCE(cnt.n, 0) AS n
      FROM public.wines w
    ...
    cb4 AS (
      SELECT w.id AS wbid, w.name AS wbname, w.producer AS wbprod, w.vintage AS wbvin, w.region AS wbreg, w.label_image_url AS wblab,
             ...
             EXP(-ABS(v_a_elo - u.elo) / v_tau) AS elo_close,
             (1.0 / SQRT(1.0 + u.n)) AS infogain,
             CASE WHEN rw4.wine_id IS NOT NULL THEN 0.5 ELSE 0.0 END AS cooldown
      FROM public.wines w
      JOIN uws4 u ON u.wid = w.id
    ...
    SELECT sb4.wbid, sb4.wbname, sb4.wbprod, sb4.wbvin, sb4.wbreg, sb4.wblab
    INTO v_b_id, v_b_name, v_b_producer, v_b_vintage, v_b_region, v_b_label
    FROM sb4 ORDER BY sb4.sc DESC LIMIT 1;
  END IF;

  -- Final fallback: random same category
  IF v_b_id IS NULL THEN
