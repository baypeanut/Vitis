...
//  Pari
//
//  Tracks password-recovery flow from deep link. Handle pari://auth/reset → show NewPasswordView.
//
...
final class AuthRecoveryState: ObservableObject {
    ...
    /// Call when app receives a URL (e.g. pari://auth/reset#...). Pass to Supabase to process recovery tokens.
    /// Redirect URL must match Supabase Dashboard → Auth → URL Configuration → Redirect URLs.
    /// Posts pariDeepLinkResetPassword so any login/forgot sheets dismiss before we present NewPasswordView.
    func handleIncomingURL(_ url: URL) {
        ...
        Task {
            ...
            NotificationCenter.default.post(name: .pariDeepLinkResetPassword, object: nil)
            showNewPasswordView = true
        }
    }

    private func isRecoveryURL(_ url: URL) -> Bool {
        guard url.scheme == "pari" else { return false }
        let host = url.host ?? ""
        let path = url.path
