...
struct WineTwinsView: View {
    ...
    private func similarityBadge(_ sim: TasteSimilarity) -> some View {
        HStack(spacing: 4) {
            ...
            Text(sim.displayText)
                .font(PariTheme.uiFont(size: 12, weight: .medium))
        }
    ...
    private func badgeColor(_ sim: TasteSimilarity) -> Color {
        ...
        case 80...100: return PariTheme.accent(for: colorScheme)
        case 60..<80:  return PariTheme.accentWine(for: colorScheme)
        default:       return PariTheme.secondaryText(for: colorScheme)
        }
    }
}
...
struct TasteTwinBadge: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 3)
        .background(PariTheme.accent(for: colorScheme).opacity(0.12))
        .clipShape(Capsule())
    }
}
